...
def MtgCardInfo(local_json = ''):
    for set_code, mtg_set in MtgJson(local_json).items():
        for card in mtg_set['cards']:
            ...
            colors = ','.join(card['colors']) if 'colors' in card else None
            card_type = card['type'] if 'type' in card else None
            supertypes = ','.join(card['supertypes']) if 'supertypes' in card else None
            types = ','.join(card['types']) if 'types' in card else None
            subtypes = ','.join(card['subtypes']) if 'subtypes' in card else None
            ...
            yield (name, mana_cost, cmc, colors, card_type, supertypes, types, subtypes, rarity, artist, set_code, set_number, power, toughness, multiverseid)
